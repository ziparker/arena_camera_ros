...
namespace Arena
{
	 ...
	 * retrieving and requeuing does not allocate or deallocate memory, but simply
	 * moves buffers around the acquisition engine. Creating, copying, and
	 * converting an image with the image factory allocates and deallocates memory
	...
	public:
		 ...
		 * designed to be generic in order to integrate image data from a variety of
		 * sources.
		 *
		 ...
		 *
		 * <B> Copy </B> creates a deep copy of an image (Arena::IImage) from another
		 * image. Images created with the image factory must be destroyed
		 * (Arena::ImageFactory::Destroy) when no longer needed.
		 *
		 * When copying an image, the image factory allocates memory for the new
		 * image. As such, images created by copying an image with the image factory
		 ...
		 * format. In doing so, it creates a completely new image, similar to a deep
		 * copy but with a different pixel format. Images created with the image
		 * factory must be destroyed (Arena::ImageFactory::Destroy) when no longer
		 ...
		 * \endcode
		 *
		 * The list of supported pixel formats can be found in the software node map
		 ...
		 * conversion to succeed, both the source and destination pixel formats must
		 * be supported. Bayer formats are supported as source formats only.
		 *
		 * @warning 
		 *  - Images from the image factory must be destroyed 
		 *  - Images from a device should be requeued 
		 ...
		 *  - none
		 *
		 * <B> Destroy </B> cleans up an image (Arena::IImage) and deallocates its
		 * memory. It must be called on any image created by the image factory
		 ...
		 * converted (Arena::ImageFactory::Convert), must be destroyed to deallocate
		 * their memory; otherwise, memory will leak. It is important that <B>
		 * Destroy </B> only be called on images from the image factory, and not on
		 * those retrieved from a device (Arena::IDevice).
		 *
		 * @warning 
		 *  - Images from the image factory must be destroyed 
		 *  - Images from a device should be requeued
