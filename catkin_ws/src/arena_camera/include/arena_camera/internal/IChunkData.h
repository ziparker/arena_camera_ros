...
namespace Arena
{
	 ...
	 * Chunk data must first be activated ('ChunkModeActive'). Each specific chunk
	 * must then be selected and enabled ('ChunkSelector', 'ChunkEnable').
	 *
	 * \code{.cpp}
	 ...
	 * 		CEnumEntryPtr pPixelFormat = pChunkSelector->GetEntryByName("PixelFormat");
	...
	public:
		 ...
		 *  - Null on failure
		 *
		 * <B> GetChunk </B> gets a specified chunk, returning it as a node in order
		 * to preserve metadata related to the chunk.
		 *
		 * Internally, chunk data objects have an internal node map and a chunk
		 * adapter. These allow chunk information to be processed and read as nodes
		 * (GenApi::INode) using GenICam and GenApi.
		 *
		 * There is a chance that incomplete images have garbage data in place of
		 * expected chunk data. If this is the case, it is still possible to attempt
		 * chunk retrieval. Invalid chunks return as null.
		 *
		 * \code{.cpp}
		 * 	// printing the timestamp from chunk
		 * 	{
		 * 		Arena::IChunkData* pChunkData = pImage->AsChunkData();
		 * 		if (!pChunkData->IsIncomplete())
		 * 		{
		 * 			GenApi::CIntegerPtr pChunkTimestamp = pChunkData->GetChunk("ChunkTimestamp");
		 * 			std::cout << "Timestamp: " << pChunkTimestamp->GetValue() << std::endl;
		 * 		}
		 * 	}
		 * \endcode
		 *
		 * Chunk data must meet three criteria to provide relevant data. Chunk mode
		 * must be activated ('ChunkModeActive'), the chunk must be enabled
		 ...
		 * excluding transport layer protocol leaders. It takes no inputs and returns
		 * the size of the data as output.
		 *
		 * The return value of <B> GetSizeFilled </B> should always be the same as
		 * the return value of <B> GetSizeOfBuffer </B>
		 ...
		 * GetSizeFilled </B> returns the size of the buffer, which is set according
		 * to the expected payload size ('PayloadSize').
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 *  - Properties lazily instantiated if buffer retrieved from device 
		 ...
		 * interpreted as chunk data. Calling <B> HasChunkData </B> from chunk data
		 * returns true.
		 *
		 ...
		 * only possible if the payload contains image data not packaged as a chunk.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued
		 *
