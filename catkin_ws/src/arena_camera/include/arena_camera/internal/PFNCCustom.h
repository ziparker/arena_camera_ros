...
namespace Arena
{
	 ...
	 *
	 * <B> GetBitsPerPixel </B> get the number of bits per pixel of the image from
	 * the integer value of the pixel format (PfncFormat).
	 *
	 * Pixel format values are determined by the PFNC (Pixel Format Naming
	 * Convention) specification. The PFNC assigns a name and number to each pixel
	 * format helping to standardize pixel formats. The number of bits per pixel
	 * can be found in each integer at bytes 5 and 6 (mask 0x00FF0000). The pixel
	 * format can be determined by the integer using the GetPixelFormatName
