...
namespace Arena
{
	 ...
	 * find, filter, and create devices on a specific interface on the host.
	 *
	 ...
	 *
	 * The list of InterfaceInfos is returned as a std::vector in order to make use
	 * of STL filter and search tools. The following code snippet demonstrates
	 * searching for an interface with a specific Mac address:
	 *
	 * \code{.cpp}
	...
	public:
		 ...
		 * checked through the main node map (Arena::ISystem::GetTLInterfaceNodeMap,
		 ...
		 *
		 * <B> SubnetMaskStr </B> gets the subnet mask of an interface on the host,
		 * returning it as a string.
		 *
		 * Both <B> SubnetMask </B> and <B> SubnetMaskStr </B> represent the same
		 * subnet mask. Where <B> SubnetMask </B> returns the value in its raw
		 ...
		 *
		 * <B> MacAddressStr </B> gets the MAC address of an interface on the host,
		 * returning it as a string.
		 *
		 * Both <B> MacAddress </B> and <B> MacAddressStr </B> represent the same MAC
		 * address. Where <B> MacAddress </B> returns the value in its raw integer
