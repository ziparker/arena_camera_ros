...
namespace Arena
{
	 ...
	 * created, copied, and converted via the image factory (Arena::ImageFactory).
	 *
	 * \code{.cpp}
	 ...
	 *  - timestamps (Arena::IImage::GetTimestamp, Arena::IImage::GetTimestampNs)
	 *
	...
	public:
		 ...
		 * information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the width is populated by the acquisition engine payload leader.
		 * The device itself is not queried as this data is present in the image
		 * data. If the image was created by the image factory, the width is
		 * populated by the arguments.
		 *
		 * @warning 
		 ...
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the height is populated by the acquisition engine payload leader.
		 * The device itself is not queried as this data is present in the image
		 * data. If the image was created by the image factory, the height is
		 * populated by the arguments.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 ...
		 *
		 * <B> GetOffsetX </B> gets the offset of the image along the X-axis. Images
		 * are self-describing, so the device does not need to be queried to get this
		 * information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the offset X is populated by the acquisition engine payload
		 * leader. The device itself is not queried as this data is present in the
		 * image data. If the image was created by the image factory, the offset X is
		 * populated by the arguments.
		 *
		 ...
		 * If the image is created from parameters, the offset X will be set to 0, no matter
		 * its original value.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 ...
		 *
		 * <B> GetOffsetY </B> gets the offset of the image along the Y-axis. Images
		 * are self-describing, so the device does not need to be queried to get this
		 * information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the offset Y is populated by the acquisition engine payload
		 * leader. The device itself is not queried as this data is present in the
		 * image data. If the image was created by the image factory, the offset Y is
		 * populated by the arguments.
		 *
		 * The image factory can create an image from another image (Arena::IImage)
		 * or from a minimal set of parameters (data, width, height, pixel format).
		 * If the created from parameters, the offset Y will be set to 0, no matter
		 * its original value.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 ...
		 * Images are self-describing, so the device does not need to be queried to
		 * get this information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the padding X is populated by the acquisition engine payload
		 * leader. The device itself is not queried as this data is present in the
		 * image data. If the image was created by the image factory, the padding X
		 * is populated by the arguments.
		 *
		 * The image factory can create an image from another image (Arena::IImage)
		 * or from a minimal set of parameters (data, width, height, pixel format).
		 * If the image was created from parameters, the padding X will be set to 0,
		 ...
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the padding Y is populated by the acquisition engine payload
		 * leader. The device itself is not queried as this data is present in the
		 * image data. If the image was created by the image factory, the padding Y
		 * is populated by the arguments.
		 *
		 * The image factory can create an image from another image (Arena::IImage)
		 * or from a minimal set of parameters (data, width, height, pixel format).
		 * If the image was created from parameters, the padding Y will be set to 0,
		 * no matter its original value.
		 *
		 * Padding Y specifically refers to the number of bytes padding the end of an
		 * image. This number will not affect the pitch/stride/step of an image.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 *  - Properties lazily instantiated if buffer retrieved from device 
		 ...
		 *
		 * <B> GetPixelFormat </B> gets the pixel format (PfncFormat) of the image,
		 * as defined by the PFNC (Pixel Format Naming Convention). Images are
		 * self-describing, so the device does not need to be queried to get this
		 * information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the pixel format is populated by the acquisition engine payload
		 * leader. The device itself is not queried as this data is present in the
		 * image data. If the image was created by the image factory, the pixel
		 * format is populated by the arguments.
		 *
		 ...
		 * from the integer value of the pixel format (PfncFormat). Internally, a
		 * public helper funciton is called (Arena::GetBitsPerPixel).
		 *
		 * Pixel format value are determined by the PFNC (Pixel Format Naming
		 * Convention) specification. The PFNC assigns a name and number to each
		 * pixel format helping to standardize pixel formats. The number of bits per
		 ...
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the pixel endianness is populated by the acquisition engine
		 * payload leader. The device itself is not queried as this data is present
		 * in the image data. If the image was created by the image factory, the
		 * pixel endianness is populated by the arguments.
		 *
		 * The image factory can create an image from another image (Arena::IImage)
		 * or from a minimal set of parameters (data, width, height, pixel format).
		 ...
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 * the image factory (Arena::ImageFactory). If the image was retrieved from a
		 * device, the timestamp is populated by the acquisition engine payload
		 * leader. The device itself is not queried as this data is present in the
		 * image data. If the image was created by the image factory, the timestamp
		 * is populated by the arguments.
		 *
		 * The image factory can create an image from another image (Arena::IImage)
		 * or from a minimal set of parameters (data, width, height, pixel format).
		 * If the image was created from parameters, the timestamp will be set to 0,
		 * no matter its original value.
		 *
		 ...
		 *  - Timestamp of the image in nanoseconds
		 *
		 * <B> GetTimestampNs </B> gets the timestamp of the image in nanoseconds.
		 * Images are self-describing, so the device does not need to be queried to
		 * get this information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 ...
		 *
		 * <B> GetData </B> returns a pointer to the beginning of the image's payload
		 * data. The payload may include chunk data.
		 *
		 * The returned data only includes payload data, not transport layer protocol
		 * leaders, which is handled internally. The pointer can be used in
		 * conjunction with size getters (Arena::IBuffer::GetSizeFilled) to read,
		 * process, and pass the data around. The data may include image data
		 ...
		 * same. <B> GetSizeFilled </B> returns the number of bytes received whereas
		 * <B> GetSizeOfBuffer </B> returns the size of the buffer, which can either
		 * be allocated by the user or calculated by Arena
		 ...
		 *
		 * <B> GetPayloadSize </B> retrieves the intended size of the payload. This
		 * is similar to the retrieved payload size (Arena::IBuffer::GetSizeFilled),
		 * but different in that missed data is included. This returns the same as
		 * the SFNC feature by the same name ('PayloadSize').
		 *
		 * @warning 
		 ...
		 * (Arena::IDevice::GetNodeMap, 'PayloadSize').
		 *
		 * The payload size is calculated at the beginning of the stream and cannot
		 * be recalculated until the stream has stopped. Because of this, features
		 * that can affect payload size ('Width', 'Height', 'PixelFormat') become
		 * unwritable when the stream has started.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 *  - Properties lazily instantiated if buffer retrieved from device 
		 ...
		 * buffers.
		 *
		 * Frame IDs start at '1' and continue until either 65535 (16-bit) or 2^64-1
		 * (64-bit), at which point they roll over back to '1'. The frame ID should
		 * never be '0'. In order to use 64-bit frame IDs, the device must support
		 * GigE Vision 2.0. Simply enable the extended ID mode feature
		 ...
		 *
		 * The payload type indicates how to interpret the data stored in the buffer
		 ...
		 *  - Otherwise, false
		 *
		 * <B> HasImageData </B> returns whether or not a payload can be interpreted
		 * as image data. Calling <B> HasImageData </B> from an image returns true.
		 *
		 ...
		 *  - Otherwise, false
		 *
		 * <B> HasChunkData </B> returns whether or not a payload can be interpreted
		 * as chunk data.
		 *
		 ...
		 * - Null on failure
		 *
		 * <B> AsChunkData </B> casts the image to chunk data (Arena::IChunkData).
		 * This is only possible if the payload contains chunk data.
		 *
		 * @warning 
		 * - Causes undefined behaviour if buffer requeued 
		 *
		 ...
		 *  - Otherwise, false
		 *
		 * <B> IsIncomplete </B> returns whether or not a buffer's payload data is
		 * complete.
		 *
		 * Error handling may be required in the case that the data is incomplete. An
		 * incomplete image signifies that the data size
		 ...
		 *
		 * The number of missed packets may be discovered through the stream node map
		 ...
		 * given buffer.
		 *
		 * A buffer may be missing data if the buffer to hold the data is too small.
		 * This happens when the size of the buffer (Arena::IBuffer::GetSizeOfBuffer)
		 * does not match the expected data size ('PayloadSize'). This function will
		 * also return true when checking whether the data is larger than the buffer
		 ...
		 *  - Otherwise, false
		 *
		 * <B> DataLargerThanBuffer </B> returns whether or not a buffer's payload
		 * data is too larger for the buffer.
		 *
		 * A buffer may be missing data if the buffer to hold the data is too small.
		 * This happens when the size of the buffer (Arena::IBuffer::GetSizeOfBuffer)
		 * does not match the expected data size ('PayloadSize'). This function will
		 ...
		 *  - Otherwise, false
		 *
		 * <B> VerifyCRC </B> calculates the CRC of an image's data and verifies it
		 * against the CRC value sent from the device. This helps verify that no data
		 * has been changed or missed during transmission. This function calls a
		 * global helper function to calculate the CRC (Arena::CalculateCRC32).
		 *
		 ...
		 * @warning 
		 *  - May throw GenICam::GenericException or other derived exception 
		 *  - Throws if chunk data disabled or not present, or CRC chunk disabled
		 *
		 * @see 
		 *  - Arena::CalculateCRC 
		 *  - Arena::IImage
