...
namespace Arena
{
	...
	public:
		 ...
		 *
		 * More specifically, for GigE devices, this flag is set to false when the
		 * Arena is not able to refresh the heartbeat on the device. If an operation
		 * times out more than 3 times the device will be flagged as not connected.
		 *
		 * @see 
		 ...
		 *  - none
		 *
		 * <B> StartStream </B> causes the device to begin streaming image/chunk data
		 ...
		 *
		 * Basically, calling <B> StartStream </B> prepares and starts the underlying
		 * streaming engine. The streaming engine primarily consists of a number of
		 * buffers, an input and an output queue, and a worker thread to run off of
		 * the main thread. All buffers are first placed in the input queue. When a
		 * buffer reaches its turn, it is filled with data. Once complete, it is
		 * moved to the output queue. At this point a buffer might be retrieved by
		 * the user (Arena::IDevice::GetBuffer, Arena::IDevice::GetImage) and then
		 * returned to the input queue (Arena::IDevice::RequeueBuffer).
		 *
		 * More specifically, <B> StartStream </B>: 
		 *  - allocates and announces a number of buffers according to the numBuffers
		 *    parameter 
		 ...
		 *  - starts the worker thread and begins listening for packets related to
		 *    the acquisition engine 
		 *  - has the device lock out certain features (e.g. 'Width', 'Height') that
		 *    cannot be changed during the stream 
		 *  - executes the 'AcquisitionStart' feature in order to have the device
		 ...
		 * \endcode
		 *
		 * All stream configurations must be completed before starting the stream.
		 * This includes, among other things, the buffer handling mode
		 * ('StreamBufferHandlingMode') found on the stream node map
		 * (Arena::IDevice::GetTLStreamNodeMap). Setting the buffer handling mode
		 * configures what the streaming engine does with buffers as they are filled
		 * and moved between queues. There are three modes to choose from: 
		 *  - 'OldestFirst' is the default buffer handling mode. As buffers are
		 *    filled with data, they get pushed to the back of the output queue. When
		 *    a buffer is requested (Arena::IDevice::GetBuffer,
		 *    Arena::IBuffer::GetImage), the buffer at the front of the queue is
		 *    returned. If there are no input buffers available, the next incoming
		 *    image is dropped and the lost frame count ('StreamLostFrameCount') is
		 *    incremented. 
		 *  - 'OldestFirstOverwrite' is similar to 'OldestFirst' except for what
		 *    happens when there are no input buffers. Instead of dropping an image,
		 *    the oldest buffer in the output queue gets returned to the input queue
		 *    so that its data can be overwritten. 
		 *  - 'NewestOnly' only ever has a single buffer in the output queue. If a
		 *    second buffer gets placed into the output queue, the older buffer gets
		 *    returned to the back of the input queue. If there are no input buffers
		 *    available, the next image is dropped and the lost frame count
		 ...
		 *  - executes the 'AcquisitionStop' feature in order to stop the device from
		 *    sending packets 
		 *  - has the device unlock features that had been locked for streaming (e.g.
		 *    'Width', 'Height') 
		 ...
		 * @return 
		 *  - Type:: Arena::IImage* 
		 *  - Pointer to the next buffer in the output queue if it contains image
		 *    data 
		 *  - Otherwise, null
		 *
		 * <B> GetImage </B> retrieves an image (Arena::IImage) from the device. It
		 * must be called after the stream has started (Arena::IDevice::StartStream)
		 * and before the stream has stopped (Arena::IDevice::StopStream). Retrieved
		 * images must be requeued (Arena::IDevice::RequeueBuffer).
		 *
		 ...
		 * GetImage </B> calls <B> GetBuffer </B>, which returns a a buffer. It then
		 * attempts to cast the buffer to an image before returning it. If the data
		 * in the buffer is that of an image, the cast succeeds and the image is
		 * returned successfully. If not, the cast fails and NULL is returned.
		 *
		 * When called, <B> GetBuffer </B> checks the output queue for image/chunk
		 * data, grabbing the first buffer in the queue. If nothing is in the output
		 * queue, the call will wait until something arrives. If nothing arrives
		 * before expiration of the timeout, a GenICam::TimeoutException is thrown.
		 *
		 * <B> GetBuffer </B> is a blocking call. If it is called with a timeout of
		 * 20 seconds and nothing arrives in the output queue, then its thread will
		 * be blocked for the full 20 seconds. However, as the timeout is a maximum,
		 * as soon as something arrives in the output queue, it will be returned, not
		 * waiting for the full timeout. A timeout value of 0 ensures the call will
		 * not block, throwing instead of waiting if nothing is in the output queue.
		 *
		 * <B> GetImage </B> does not verify image quality. If an incomplete image is
		 * returned, the call will not throw and the image will be returned as is. It
		 * is recommended to verify each image's completion
		 ...
		 * order to prevent starvation of the acquisition engine. If image data is
		 * needed for a longer period of time (i.e. for processing), it is
		 * recommended to copy the data (Arena::ImageFactory::Create) and requeue the
		 * buffer.
		 *
		 * @warning 
		 *  - Does not guarantee valid image data 
		 *  - Returns NULL for non-image formats 
		 *  - Images should be requeued to prevent starvation of the acquisition
		 *    engine 
		 *  - May throw GenICam::GenericException or other derived exception
		 *
		 * @see 
		 ...
		 *
		 * <B> GetBuffer </B> retrieves a buffer (Arena::IBuffer) from the device. It
		 * must be called after the stream has started (Arena::IDevice::StartStream)
		 * and before the stream has stopped (Arena::IDevice::StopStream). Retrieved
		 * images must be requeued (Arena::IDevice::RequeueBuffer).
		 *
		 * The data returned from <B> GetBuffer </B> may represents different payload
		 * types: an image without chunk, an image with chunk, or just chunk data.
		 * Note that a buffer of chunk data payload type may contain image data, but
		 * cannot be cast to an image because the image data is treated as a chunk.
		 * The payload type can be queried (Arena::IBuffer::GetPayloadType), which
		 * returns an enum (Arena::EBufferPayloadType): 
		 ...
		 *
		 * <B> GetBuffer </B> is essentially the same call as <B> GetImage </B>, just
		 * more generic. Internally, <B> GetImage </B> calls <B> GetBuffer </B> and
		 * then casts the returned buffer to an image. <B> GetBuffer </B> does not
		 * perform the cast, instead returning the raw buffer data. The buffer object
		 * may be queried for its type (Arena::IBuffer::GetPayloadType,
		 * Arena::IBuffer::HasImageData, Arena::IBuffer::HasChunkData) and cast to
		 ...
		 * image data is needed for a longer period of time (i.e. for processing), it
		 * is recommended to copy the data (Arena::ImageFactory::Create) and requeue
		 * the buffer.
		 *
		 * @warning 
		 ...
		 *  - none
		 *
		 * <B> RequeueBuffer </B> relinquishes control of a buffer (Arena::IBuffer,
		 * Arena::IImage, Arena::IChunkData) back to Arena. It must be called after a
		 * buffer has been retrieved (Arena::IDevice::GetBuffer,
		 * Arena::IDevice::GetImage).
		 *
		 * When called, <B> RequeueBuffer </B> deallocates any lazily instantiated
		 * memory and returns the internal buffer to the acquisition engine's input
		 * queue, where it can be filled with new data. If enough buffers have been
		 * removed from the acquisition engine (i.e. not requeued), it is possible to
		 * starve the acquisition engine. If this happens and depending on the buffer
		 * handling mode (Arena::IDevice::GetTLStreamNodeMap,
		 * 'StreamBufferHandlingMode'), data may start being dropped or buffers may
		 * start being recycled.
		 *
		 * Best practices recommends that buffers be requeued as soon as they are no
		 * longer needed. If image data is needed for a longer period of time (i.e.
		 * for processing), it is recommended to copy the data
		 * (Arena::ImageFactory::Create) and requeue the buffer.
		 *
		 * It is important to only call <B> RequeueBuffer </B> on buffers retrieved
		 * from a device (Arena::IDevice::GetBuffer, Arena::IDevice::GetImage), and
		 * not on images created through the image factory (Arena::ImageFactory).
		 *
		 * \code{.cpp}
		 ...
		 * @warning 
		 *  - Buffers should be requeued 
		 *  - Used only on buffers retrieved from a device, not on images created
		 *    through the image factory 
		 *  - May throw GenICam::GenericException or other derived exception
		 *
		 * @see 
		 *  - Arena::IBuffer 
		 ...
		 * registration information. When an event fires, the events engine takes an
		 * event buffer from the input queue, stores all relevant data, and places it
		 * in the output queue. When <B> WaitOnEvent </B>
		 * (Arena::IDevice::WaitOnEvent) is called, the engine takes the buffer from
		 * the output queue, processes its data, and returns it to the input queue.
		 *
		 ...
		 * channel. Arena processes event data internally, which it attaches to the
		 * device node map (not the GenTL device node map) using a
		 * GenApi::EventAdapter. The appropriate nodes are then updated in the node
		 * map. It can be helpful to incorporate callbacks (GenApi::Callback) to be
		 ...
		 * listening for messages, shutting it down and cleaning it up. It should be
		 * called only after the events infrastructure has been initialized
		 * (Arena::IDevice::InitializeEvents) and after all events have been
		 * processed (Arena::IDevice::WaitOnEvent).
		 *
		 * Roughly speaking, <B> DeinitializeEvents </B> takes all necessary steps to
		 * undoing and cleaning up the event infrastructure's initialization
		 ...
		 *  - unregister all buffers and clean up memory that was allocated for them
		 *
		 * @see 
		 ...
		 *  - none
		 *
		 * <B> WaitOnEvent </B> waits for an event to occur in order to process its
		 ...
		 * complexities. When an event occurs, the data is stored in an event buffer
		 * and placed on the output queue. <B> WaitOnEvent </B> causes the data to be
		 * processed, updating all relevant nodes appropriately. This is why <B>
		 * WaitOnEvent </B> does not return any event data; when the data is
		 * processed, nodes are updated, which can then be queried for information
		 * through the node map. This is also why callbacks (GenApi::Callback) work
		 * so well with the events infrastructure; they provide a method of accessing
		 * nodes of interest as they change.
		 *
		 * \code{.cpp}
		 ...
		 * \endcode
		 *
		 * When called, <B> WaitOnEvent </B> checks the output queue for event data
		 * to process, grabbing the first buffer from the queue. If nothing is in the
		 * output queue, the call will wait until an event arrives. If nothing
		 * arrives before expiration of the timeout, a GenICam::TimeoutException is
		 * thrown.
		 *
		 * <B> WaitOnEvent </B> is a blocking call. If it is called with a timeout of
		 * 20 seconds and nothing arrives in the output queue, then its thread will
		 * be blocked for the full 20 seconds. However, as the timeout is a maximum,
		 * when an event arrives in the output queue, the event will process, not
		 * waiting for the full timeout. A timeout value of 0 ensures the call will
		 * not block, throwing instead of waiting if nothing is in the output queue.
		 *
		 * @warning 
		 ...
		 * nodes (GenApi::INode).
		 *
		 * As a simple getter, <B> GetNodeMap </B> retrieves this node map without
		 * doing anything to initialize, manage, or maintain it. This node map is
		 * initialized when the device is created (Arena::ISystem::CreateDevice) and
		 * deinitialized when the device is destroyed
		 ...
		 * permitted.
		 *
		 * The node map is built from XMLs stored on the device itself. The XML is
		 * downloaded and parsed before constructing and initializing the node map.
		 * This node map describes and provides access to all device features, and
		 * may vary from device to device. Lucid products conform to the SFNC 2.3
		 * specification. Note that both chunk data and event data are updated on
		 * this node map.
		 *
		 * Arena provides access to five different node maps. This one comes from the
		 * device and describes all its features. Please check device documentation
		 * for more information regarding these features. 
		 *  - device (Arena::IDevice::GetNodeMap)
		 *
		 * The other four node maps describe and provide access to information and
		 * settings through the software rather than the device. 
		 ...
		 *
		 * The most noticable difference between the two device node maps is that the
		 * GenTL device node map has only a small set of features compared to the
		 * main node map. There are a few features that overlap. For example, the
		 * difference between retrieving the serial number ('DeviceSerialNumber') is
		 * that using the main node map queries the camera directly whereas the GenTL
		 * node map queries a set of information cached at device creation. The
		 * result, however, should be the same. Basically, the GenTL node map queries
		 * the software for information whereas the main node map queries the device.
		 *
		 * @warning 
		 *  - Provides access to main node map, not to be confused with the GenTL
		 *    device node map 
		 *  - May throw GenICam::GenericException or other derived exception
		 *
		 * @see 
		 ...
		 *
		 * <B> GetTLDeviceNodeMap </B> retrieves the already initialized GenTL device
		 * node map (GenApi::INodeMap), used to access a subset of cached device
		 * related nodes (GenApi::INode).
		 *
		 * As a simple getter, <B> GetTLDeviceNodeMap </B> retrieves this node map
		 * without doing anything to initialize, manage, or maintain it. This node
		 * map is initialized when the device is created
		 * (Arena::ISystem::CreateDevice) and deinitialized when the device is
		 * destroyed (Arena::ISystem::DestroyDevice). Because node maps are cleaned
		 * up internally, retrieving multiple pointers to the same node map is
		 * permitted.
		 *
		 ...
		 *  - device (Arena::IDevice::GetNodeMap)
		 *
		 * The other four, including this one, node maps describe and provide access
		 * to information and settings through the software rather than the device. 
		 *  - system GenTL (Arena::ISystem::GetNodeMap) 
		 *  - stream GenTL (Arena::IDevice::GetTLStreamNodeMap) 
		 *  - device GenTL (Arena::IDevice::GetTLDeviceNodeMap) 
		 *  - interface GenTL (Arena::IDevice::GetTLInterfaceNodeMap)
		 ...
		 *
		 * <B> GetTLStreamNodeMap </B> retrieves the already initialized GenTL stream
		 * node map (GenApi::INodeMap), used to access stream related nodes
		 * (GenApi::INode).
		 *
		 * As a simple getter, <B> GetTLStreamNodeMap </B> retrieves this node map
		 * without doing anything to initialize, manage, or maintain it. This node
		 * map is initialized when the device is created
		 * (Arena::ISystem::CreateDevice) and deinitialized when the device is
		 * destroyed (Arena::ISystem::DestroyDevice). Because node maps are cleaned
		 ...
		 *  - stream information such as the payload size or whether the device is
		 *    currently streaming 
		 *  - stream statistics such as lost frames, announced buffers, or missed
		 *    packets
		 *
		 * Arena provides access to five different node maps. This one comes from the
		 * device and describes all its features. Please check device documentation
		 ...
		 * nodes (GenApi::INode).
		 *
		 * As a simple getter, <B> GetTLInterfaceNodeMap </B> retrieves this node map
		 * without doing anything to initialize, manage, or maintain it. This node
		 * map is initialized when the device is created
		 * (Arena::ISystem::CreateDevice) and deinitialized when the device is
		 * destroyed (Arena::ISystem::DestroyDevice). Because node maps are cleaned
