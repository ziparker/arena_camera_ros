...
namespace Arena
{
	 ...
	 * compared for equality. If the values are the same, then the transmission is
	 * deemed successful; if different, then something in the transmission went
	 * wrong.
	 *
	 ...
	 *
	 * Buffers are the most generic form of acquisition engine data retrieved from
	 * a device. They are acquired and requeued via devices (Arena::IDevice).
	 *
	 * \code{.cpp}
	 ...
	 * buffer class: 
	 *  - buffer and payload information like payload (Arena::IBuffer::GetData),
	 *    payload and buffer size (Arena::IBuffer::GetSizeFilled,
	 *    Arena::IBuffer::GetSizeOfBuffer), and frame ID
	 *    (Arena::IBuffer::GetFrameID) 
	 *  - type information like payload type (Arena::IBuffer::GetPayloadType) and
	 *    whether the payload has image and/or chunk data
	 *    (Arena::IBuffer::HasImageData, Arena::IBuffer::HasChunkData) 
	 *  - casting helpers (Arena::IBuffer::AsImage, Arena::IBuffer::AsChunkData) 
	 *  - error information (Arena::IBuffer::IsIncomplete, 
	 *  - Arena::IBuffer::DataLargerThanBuffer)
	 *
	 * Retrieving data as a buffer can be preferable to an image or chunk data, as
	 * it can allow data to be received and treated generically before further
	 * processing.
	 *
	 * \code{.cpp}
	...
	public:
		 ...
		 *
		 * <B> GetData </B> retrieves a pointer to the buffer's payload data. This
		 * data may include image data, chunk data, or both.
		 *
		 * To check the type of data returned, image (Arena::IBuffer::HasImageData)
		 * and chunk data (Arena::IBuffer::HasChunkData) can be checked for
		 * specifically, or the payload type (Arena::EBufferPayloadType) can be
		 * retrieved.
		 *
		 ...
		 * process, and pass the data around.
		 *
		 * @warning 
		 ...
		 *
		 * <B> GetSizeFilled </B> retrieves the size of the payload data, excluding
		 * transport layer protocol leaders. The payload data may include image data,
		 * chunk data, or both.
		 *
		 ...
		 *  - Otherwise, false
		 *
		 * <B> HasImageData </B> returns whether or not a buffer's payload has data
		 * that may be interpreted as image data.
		 *
		 ...
		 *  - Otherwise, false
		 *
		 * <B> HasChunkData </B> returns whether or not a buffer's payload that may
		 * be interpreted as chunk data.
		 *
		 ...
		 *  - Null on failure
		 *
		 * <B> AsChunkData </B> casts the buffer to a chunk data (Arena::IChunkData).
		 ...
		 * cumulative count of all missed packets, and does not necessarily reflect
		 * the number of missed packets for any given buffer.
		 *
		 ...
		 * does not match the expected data size (Arena::IDevice::GetTLStreamNodeMap,
		 * 'PayloadSize'). This function will also return true when checking whether
		 * the data is larger than the buffer (Arena::IBuffer::DataLargerThanBuffer).
		 *
		 * @warning 
		 *  - Causes undefined behavior if buffer requeued 
		 *  - Properties lazily instantiated if buffer retrieved from device 
		 ...
		 *  - Otherwise, false
		 *
		 * <B> VerifyCRC </B> calculates the CRC of a buffer's data and verifies it
		 * against the CRC value sent from the device. This helps verify that no data
		 * has been changed or missed during a transmission. This function calls a
